...
#실습 4-1
CREATE TABLE `Member` (
			`uid`		VARCHAR(10) IS NOT NULL,
			`name`	VARCHAR(10) IS NOT NULL,
			`hp`		CHAR(13) IS NOT NULL,
			`pos`		VARCHAR(10) IS NOT NULL,
			`dep`		INT,
			`rdate`	DATETIME IS NOT NULL
			);

CREATE TABLE `Department` (
			`depNo`		INT IS NOT NULL,
			`name`		VARCHAR(10) IS NOT NULL,
			`tel`			CHAR(12) IS NOT NULL
			);
