...
GRANT ALL PRIVILEGES ON `Binformation_schemaank`.* TO 'admin2'@'%';
FLUSH PRIVILEGES;
...
#실습 2-15 AS 없어도 됨
SELECT SUM(`t_amount`) `입금총합`, AVG(`t_amount`) AS `입금평균` FROM `bank_transaction` WHERE `t_dist` = 1;
...
SELECT
	...
	WHERE `t_dist` = 1
	GROUP BY `t_a_no`
	HAVING `합계` >= 100000 #바뀐 이름으로(합계) 칼럼 적어야함
	ORDER BY `합계` DESC;
...
GROUP BY `c_no` # t_no 로 그루핑하면 안됨?? t_no로 그루핑하면 이미 1개 되있으니까 count할때 셀 수가 없음.
