...
INSERT INTO `Departments` VALUES (107, '흉부외과', '류성룔', '051-123-0107');
...
#Doctors #테이블 칼럼 순서 다르게 넣으려면 칼럼 지정해서 데이터 넣으면됨. `Doctors` (``, ``, ``) 이 방식으로
...
INSERT INTO `Nurses` VALUES ('N105101', '송혜고', '1984-07-15', 'F', 105, '주임', '010-4101-8407', 'song@bw.com');
...
SELECT `pat_id`, `pat_name`, `pat_jumin`, `pat_phone`, b.`doc_name`, c.`nur_name` FROM `Patients` AS a
...
WHERE `dep_no` = 103; # 진료과명이 '외과' 인걸로 찾아야 하므로 Departments까지 JOIN 하는 게 맞음. 해답이 정답임
...
SELECT * FROM
	...
	FROM 
		(SELECT
		*,
			CONCAT('19', SUBSTRING(`pat_jumin`, 1, 2)) AS `year` #주민번호에서 1번째에서 2개
		FROM `Patients`) AS a
	) AS b
...
WHERE  (YEAR(NOW()) - CONCAT(19,SUBSTR(pat_jumin,1,2))+1) BETWEEN 30 AND 40; # 생일 안 지났다고 치면 +1로 계산하는 게 맞음 / CONCAT은 갖다붙이는 의미가 있네
...
WHERE `doc_id` IS NULL; #JOIN 하면 dep_no 겹치는 애들만 나옴. LEFTJOIN으로 Doctors 테이블에 없는 애들 꺼내고 걔네 이름이 dep_manager임 

# 문제9. 김태희 간호사가 맡고 있는 모든 환자의 이름을 구분자 ^으로 해서 '강동원^고소영^이나영'과 같이 조회하시오.***
SELECT GROUP_CONCAT(`pat_name` SEPARATOR '^') AS `환자들` FROM `Patients` AS a # GROUP_CONCAT 꼭 그루핑해야만 나올 수 있는 거아님
JOIN `Nurses` AS b
ON a.nur_id = b.nur_id
WHERE `nur_name` = '김태희'; # 문제에서 김태희로 찾으라했으니까 

# 문제10. 가장 많은 환자 수를 담당하는 간호사ID, 간호사 이름, 담당환자수를 조회 하시오.
SELECT a.`nur_id`, `nur_name`, COUNT(*) `담당환자수` FROM `Patients` AS a
JOIN `Nurses` AS b
ON a.nur_id = b.nur_id
GROUP BY `nur_id`
ORDER BY `담당환자수` DESC
